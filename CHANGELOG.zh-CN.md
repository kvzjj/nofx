...
### 修复
- 止损/止盈触发价按币种 tick size 取整；此前未取整的 AI 价格会被交易所拒绝（-4114）并升级为强制平仓
- 保护性条件单改用标记价触发并开启价格保护，抵抗插针操纵和触发后的滑点
- 开新仓前撤销同方向残留保护单（如止损触发后存活的止盈腿），避免“开仓即被瞬间平掉”的竞态
- 平仓决策现在会撤销同方向的限价挂单，挂单不再能静默重建刚平掉的仓位（紧急平仓同样处理）
- 保护单遭遇确定性交易所拒绝（精度/过滤器/已触发）时不再重试，立即转入安全关闭
- 仓位消失的同一对账周期内即撤销存活的另一腿保护单，不再等待孤儿清理周期
- 孤儿保护单清理不再误删同一共享账户下其他交易员持有的仓位保护单

### 新增
- 挂单监控参数可配置（`pending_order_timeout_sec`、`pending_order_poll_sec`），轮询错误渐进退避，并在策略风控 UI 中开放
- 通过币安用户数据 WebSocket 获取实时订单更新：部分成交保护低延迟生效，REST 轮询退化为 15-60 秒的慢速兑底（大幅降低 API weight）
- 止损/止盈改为按数量保护，取代全仓平仓单：分片支持部分止盈与移动止损策略；低于交易所最小名义价值的碎片自动回退为全仓保护单
- 超额覆盖重平衡：数量型保护超过缩减后仓位时（触发时会遭拒），对账逻辑自动撤销并按精确数量重挂
...
- [安全策略](SECURITY.md)
- [GitHub 仓库](https://github.com/kvzjj/auaiex)
