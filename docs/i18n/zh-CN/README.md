...
- **币安实盘 + 模拟盘**: 币安合约实盘交易，内置模拟盘交易所（真实行情价格、本地模拟撮合），零风险验证策略
...
> **重要说明：** 后续更新中，NOFX 实盘交易仅支持 **币安合约（Binance Futures）**。其他交易所适配器在当前版本中不可用，且不再更新。

### 实盘交易（CEX）

| 交易所 | 状态 | 注册 (手续费折扣) |
|----------|--------|-------------------------|
| **币安合约** | ✅ 已支持 | [注册](https://www.binance.com/join?ref=NOFXENG) |

### 内置模拟盘（Paper Trading）

| 交易所 | 状态 | 所需密钥 |
|----------|--------|---------------|
| **模拟盘** | ✅ 内置 | 无需任何密钥 —— 真实行情价格 + 本地模拟撮合，详见下方[模拟盘](#模拟盘paper-trading)章节 |

### 其他交易所

| 交易所 | 状态 |
|----------|--------|
| Bybit / OKX / Bitget | ❌ 当前版本不可用（不再更新） |
| Hyperliquid / Aster DEX / Lighter | ❌ 当前版本不可用（不再更新） |

---

## 模拟盘（Paper Trading）

模拟盘是内置的模拟交易所：AI 交易员运行**与实盘完全相同的决策链路、风控体系和审计记录**，但订单按真实行情价格在本地撮合 —— 无需 API Key、不动真实资金、零风险。

### 快速开始

1. **配置 → 交易所 → 添加**，选择 **Paper Trading (Simulated)** —— 无需 API Key
2. **创建交易员**并绑定模拟账户；交易员的「初始余额」即模拟资金（默认 10,000 USDT）
3. 启动交易员 —— 仪表盘、决策日志、竞赛模式全部照常工作

### 模拟撮合规则

- 市价单按实时价格 ± 0.02% 滑点成交；taker 手续费 0.05%
- 限价单在价格穿越限价时成交；maker 手续费 0.02%
- 止损/止盈单在价格穿越触发价时按触发价平仓（交易所侧止损同样被模拟）
- 后台撮合器每 3 秒基于 WebSocket 行情评估挂单
- 模拟余额、持仓、挂单重启后保留
- 交易员卡片上的 **一键重置**（↺ 重置按钮）清空模拟状态并恢复初始资金

### 已知局限

- **未模拟资金费率** —— 跨资金费率结算点持仓的模拟盈亏略偏乐观
- 强平为近似估算；极端行情下真实交易所可能更早强平
